...
# -------------------------
SINGLE_TXN_LIMIT = 150000     # > this amount => fail
DAILY_LIMIT = 500000          # daily total per payer > this => fail
...
# -------------------------
# Transaction Pipeline with Logs (simulated Kafka)
# -------------------------
def insert_transaction_pipeline(
    tx_id: str,
    payer_id: str,
    payee_id: str,
    amount: int,
    ...
    """
    Simulated Kafka pipeline with business rules and live logs.

    - Enforces SINGLE_TXN_LIMIT and DAILY_LIMIT (per payer via Redis daily:{payer}:{date}).
    - Creates users in Postgres if missing.
    - Inserts transaction into Postgres with final_status.
    - Inserts audit into Cassandra.
    - Updates Redis balances & daily totals only when SUCCESS.
    """
...
with col1:
    ...
    if submitted:
        ...
        st.success("Pipeline simulation finished.")
...
st.markdown("---")
st.caption("Simulated Kafka → Postgres → Cassandra → Redis pipeline with step-by-step live logs.")